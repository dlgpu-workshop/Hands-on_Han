function [z_analytic,swi_analytic,swe_analytic,phe_analytic,tot_analytic]=Get_Analytic_TypeA_densities(Zmin,Zmax,angle)
...
solinitB=bvpinit(linspace(Zmin*lambda,phi_barrier,nl),[1.0,-0.01]);
solB=bvp4c(@(t,y)odefcnB(t,y,p),@(ya,yb)bcfcnB(ya,yb,p),solinitB);
xB=linspace(Zmin*lambda,phi_barrier,nl);
phi_tempB=deval(solB,xB);
phiB = phi_tempB(1,:);

solinitC=bvpinit(linspace(phi_barrier,Zmax*lambda,nl),[1.0,-0.01]);
solC=bvp4c(@(t,y)odefcnC(t,y,p),@(ya,yb)bcfcnC(ya,yb,p),solinitC);
xC=linspace(phi_barrier,Zmax*lambda,nl);
...
ne=p.n_e_inf/2.*exp((phiAnalytic-p.phi_inf)/p.Te).*(1-erf(real(sqrt((phiAnalytic-p.phi_m)/p.Te))-p.V_d/p.Vth_e));
ner=p.n_e_inf.*exp((phiAnalytic-p.phi_inf)/p.Te).*(erf(real(sqrt((phiAnalytic-p.phi_m)/p.Te))-p.V_d/p.Vth_e)+erf(p.V_d/p.Vth_e));
nne=ne+ner;
...
ni=p.n_i_inf./sqrt(1-2.*phiAnalytic/(p.Te.*p.Mi^2));
swi_analytic=ni/n0;

%% phe

nphe=p.n_p_0/2.*exp((phiAnalytic-p.phi_0)/p.Tp).*(1-erf(real(sqrt((phiAnalytic-p.phi_m)/p.Tp))));
npheca=p.n_p_0.*exp((phiAnalytic-p.phi_0)/p.Tp).*erf(real(sqrt((phiAnalytic-p.phi_m)/p.Tp)));

np=nphe+npheca;
np(nl+1:size(np,2))=nphe(nl+1:size(np,2));
