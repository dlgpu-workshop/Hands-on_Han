function [z_analytic,swi_analytic,swe_analytic,phe_analytic,tot_analytic]=Get_Analytic_TypeC_densities(Zmin,Zmax,angle)
...
ner=p.n_e_inf.*exp((phi-p.phi_inf)/p.Te).*(erf(real(sqrt((phi-p.phi_m)/p.Te))-p.V_d/p.Vth_e)+erf(p.V_d/p.Vth_e));
nne=ne+ner;
swe_analytic=-nne/n0;
...
%% phe
np=p.n_p_0/2.*exp((phi-p.phi_0)/p.Tp).*(1-erf(real(sqrt((phi-p.phi_m)/p.Tp))));
phe_analytic=-np/n0;
...
phi_hat_0=MyRootFinder(@(x)Confun(x,p),-20,0,1e-8,1e3);
a=1+2*erf(p.Vd/p.Vth_e)+erf(sqrt(-phi_hat_0/p.beta)-p.Vd/p.Vth_e);
b=p.n_p_0/2*exp(-phi_hat_0)*(1-erf(sqrt(-phi_hat_0)))-p.n_i_inf;
...
c = ...
    (   sqrt(p.beta)*exp(-(sqrt(-phi_hat_0/p.beta)-p.Vd/p.Vth_e).^2)...
        + p.Vd/p.Vth_p*sqrt(pi)*erfc(-p.Vd/p.Vth_e+sqrt(-phi_hat_0/p.beta))...
    );
d = p.n_i_inf * sqrt(2*pi*p.beta*p.me/p.mi)*p.Mi;

result = p.n_p_0 - c.*n_e_inf + d;
...
% SEA = 5; % sun elevation angle, degree
